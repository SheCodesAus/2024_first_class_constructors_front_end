...
function GiftComparisonCard(props) {
  ...
  return (
    <div className="  m-4 flex flex-col md:w-70 md:border md: border-primary-600 md:border-opacity-80">
      <div>
        <Link to={`/gift/${giftData.id}`}>
          <div className="p-4 flex justify-center ">
            <img
              src={giftData.img}
              alt={giftData.name}
              className="w-full rounded-md overflow-hidden shadow-lg p-4  bg-slate-50 border border-primary-600 border-opacity-80 md:h-64 "
            />
          </div>
        </Link>
      </div>
      ...
      <div className="flex flex-col justify-between p-4 md:p-6 ">
        ...
        <div className="space-y-6 mb-6 flex-grow">
          <h3 className="font-montserrat text-primary-100 text-h2 tracking-wider uppercase md:min-h-44 ">
            {giftData.name}
          </h3>
          <h3 className="font-montserrat  text-primary-100  text-[22px] tracking-wider">
            {giftData.price}
          </h3>
          <p className="text-primary-100 text-p tracking-wider uppercase md:min-h-96 ">
